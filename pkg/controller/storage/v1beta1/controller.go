...
// when a CacheStorageClass is changed
type clusterMapper struct {
	client client.Client
}

func (m *clusterMapper) Map(object handler.MapObject) []reconcile.Request {
	log.Info("Filter CacheStorageClass", "Namespace", object.Meta.GetNamespace(), "Name", object.Meta.GetName())
	...
	for _, database := range databases.Items {
		if (database.Spec.StorageClass.Group == "" || database.Spec.StorageClass.Group == v1beta1.CacheStorageClassGroup) &&
			(database.Spec.StorageClass.Version == "" || database.Spec.StorageClass.Version == v1beta1.CacheStorageClassVersion) &&
			(database.Spec.StorageClass.Kind == "" || database.Spec.StorageClass.Kind == v1beta1.CacheStorageClassKind) &&
			...
			log.Info("Matched CacheStorageClass", "Namespace", object.Meta.GetNamespace(), "Name", object.Meta.GetName())
...
func (m *storageFilter) Map(object handler.MapObject) []reconcile.Request {
	...
	if (database.Spec.StorageClass.Group == "" || database.Spec.StorageClass.Group == v1beta1.CacheStorageClassGroup) &&
		(database.Spec.StorageClass.Version == "" || database.Spec.StorageClass.Version == v1beta1.CacheStorageClassVersion) &&
		(database.Spec.StorageClass.Kind == "" || database.Spec.StorageClass.Kind == v1beta1.CacheStorageClassKind) {
		storageClass := &v1beta1.CacheStorageClass{}
